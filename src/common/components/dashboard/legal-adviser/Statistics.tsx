...
const Statistics: FC = () => {
  ...
  useEffect(() => {
    const getStats = async () => {
      const data = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/reports/get_statictics_by_user.php?user_type=legal_adviser&users_id=${userId}`
      );
  ...
  return (
    <>
      <div className="grid  grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-5 my-4">
