...
const KickoffCertificateForm: FC<IProps> = ({
  showModal,
  setShowModal,
  user,
}: IProps) => {
  ...
  const { mutate } = certificateQueries.createKickoffCertificate();
  ...
  const schema = Yup.object({
    ...
    kick_off: Yup.number().required("Required"),
  });

  return (
    <Modal
      label="eFCAS CLEARANCE CERTIFICATE"
      showModal={showModal}
      onClose={() => {
        setShowModal(false);
      }}
...
export default KickoffCertificateForm;
